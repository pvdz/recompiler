// customized build of the ZeParser2 parser
...
//    if (this.tok.input === '500') debugger
  ...
  parseStatement: function(inFunction, inLoop, inSwitch, labelSet, optional){
    var tok = this.tok;
    this.tok.lastToken.statementStart = true; // required to find the start of an expression from an arbitrary point in the expr
...
//      if (c > 0x66 && c < 0x72 && c != 0x69) this.parseExpressionOrLabel(inFunction, inLoop, inSwitch, labelSet); // i dunno if this is a good idea
      ...
      else if (c === 0x66 && tok.getLastValue() === 'function') {
        this.tok.lastToken.isDeclaration = true;
        this.parseFunction();
      }
  ...
  parseFunction: function(){
    ...
    if (tok.isType(IDENTIFIER)) { // name
      ...
      funcToken.nameToken = tok.lastToken;
      tok.nextPunc();
    }
    this.parseFunctionRemainder(-1, funcToken);
  },
  /**
   ...
   * @param {Function} funcToken The function keyword token (null if getter/setter)
   */
  parseFunctionRemainder: function(paramCount, funcToken){
    var tok = this.tok;
    tok.mustBeNum(0x28, false); // (
    var lhp = tok.prevToken;
    ...
    tok.mustBeNum(0x29, false); // )
    var rhp = tok.prevToken;
    var lhb = this.parseCompleteBlock(true, false, false, []); // this resets loop and switch status

    lhp.rhp = rhp;
    rhp.lhp = lhp;

    // link parens and brackets to functions
    if (funcToken) {
      funcToken.lhp = lhp;
      funcToken.rhp = rhp;
      funcToken.lhb = lhb;
      funcToken.rhb = lhb.rhb;
    }
  },
  ...
  parseBlock: function(inFunction, inLoop, inSwitch, labelSet){
    var lhb = this.tok.prevToken;
    this.parseStatements(inFunction, inLoop, inSwitch, labelSet);
    this.tok.mustBeNum(0x7d, true); // }
    var rhb = this.tok.prevToken;
    lhb.rhb = rhb;
    rhb.lhb = lhb;
    return true; // why true again?
  },
  parseCompleteBlock: function(inFunction, inLoop, inSwitch, labelSet){
    var lhb = this.tok.lastToken;
  ...
  parsePair: function(){
    ...
    if (tok.isNum(0x67) && tok.nextPuncIfString('get')) {
      if (tok.nextPuncIfType(IDENTIFIER)) this.parseFunctionRemainder(0, null);
      else this.parseDataPart();
    } else if (tok.isNum(0x73) && tok.nextPuncIfString('set')) {
      if (tok.nextPuncIfType(IDENTIFIER)) this.parseFunctionRemainder(1, null);
      else this.parseDataPart();
    } else {
      this.parseData();
    }
